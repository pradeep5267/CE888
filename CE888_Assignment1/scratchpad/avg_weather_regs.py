...
#%%
df['plant_check'] = (df['flight_mod'] - df['plant_dates']).dt.days
...
#%%
df['flight_mod'] = pd.to_datetime(df['Check Date'])
...
# %%
df.to_csv('../dataset/tmp2.csv')
#%%
#
df.to_csv('../dataset/tmp4.csv')
#%%
df.loc[df['Flight Date_x'] == pd.NaT, 'Flight Date_x'] = df['Flight Date_y']
df.to_csv('../dataset/tmp4.csv')

#%%
df['flight_mod']= np.where(df['Flight Date_x'] is pd.NaT, df['Flight Date_x'], df['Flight Date_y'])
df.to_csv('../dataset/tmp4.csv')

# df['flight_mod'] = df['Flight Date_x']
# %%
for index, row in df.iterrows():
    if row['flight_mod'] is pd.NaT:
        df.iloc[index]['flight_mod'] = df.iloc[index]['Flight Date_y']
    else:
        df.iloc[index]['flight_mod'] = df.iloc[index]['Flight Date_x']
df.to_csv('../dataset/tmp4.csv')

# %%
df['Flight Date_x'].fillna(df['Flight Date_y'], inplace=True)
df.to_csv('../dataset/tmp4.csv')

# %%
df['flight_mod'] = [df['Flight Date_y'] if x is pd.NaT else df['Flight Date_x'] for x in df['Flight Date_x']]
# df.to_csv('../dataset/tmp3.csv')
...
df = df[df.Remove != 'r']
df.drop(columns=['Unnamed: 0', 'index', 'Flight Date_x', 'Flight Date_y', 'flight_mod', 'Plant Date', 'Check Date', 'plant_dates', 'check_date', 'Remove'], inplace= True)
df.info()
# %%
import datetime
for idx, row in df.iterrows():
    df.iloc[idx]['tmp'] = datetime.datetime(df.iloc[idx]['flight_mod_year'], df.iloc[idx]['flight_mod_month'], df.iloc[idx]['flight_mod_day'])
# %%
df.info()
